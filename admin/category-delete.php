...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if ($e->errorInfo[1] === 1451) {
            redirect('categories.php', ['failure' => 'Category contains articles
            that must be moved or deleted before you can delete it']);
        } else {
            throw $e;
        }
    }
...
<main class="container admin" id="content">
    ...
    <form action="category-delete.php?id=<?= $id ?>" method="post" class="narrow">
        <p>Click confirm to delete the category <?= escape($category) ?></p>
